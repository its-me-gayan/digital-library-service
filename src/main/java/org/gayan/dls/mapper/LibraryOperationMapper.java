...
/** Author: Gayan Sanjeewa User: gayan Date: 9/22/25 Time: 10:18 PM */
@Mapper(
...
public interface LibraryOperationMapper {
  ...
  @Mapping(
      ...
      source = "book",
      qualifiedByName = "mapAvailableCopies")
  ReturnBookResponseDto buildReturnResponseDto(
      Book book, Borrower borrower, BookCopy bookCopy, BorrowingHistory borrowingHistory);
  ...
  default int mapAvailableCopies(Book book) {
    ...
    return (int)
        book.getCopies().stream()
            .filter(
                c ->
                    !Boolean.TRUE.equals(c.getIsBorrowed())
                        && c.getBookStatus().equals(BookStatus.AVAILABLE))
            .count();
  }
}
