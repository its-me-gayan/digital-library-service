...
/**
 * Service implementation that handles core library operations such as borrowing and returning
 * books. This class enforces transactional consistency, prevents double-borrowing, manages book
 * copy availability, and updates borrowing history. Author: Gayan Sanjeewa User: gayan Date:
 * 9/21/25 Time: 11:23 PM
 */
@Service
...
public class LibraryOperationServiceImpl implements LibraryOperationService {
   ...
   *
   * <p>Flow: 1. Validate that both book and borrower exist. 2. Ensure the borrower has not already
   * borrowed the same book. 3. Acquire the first available book copy using a pessimistic lock (FOR
   * UPDATE). 4. Update the copy’s status and borrower reference. 5. Persist borrowing history for
   * traceability. 6. Return a success response with borrowing details.
   *
  ...
  public ResponseEntity<ApiResponse<BorrowBookResponseDto>> borrowBook(
      BorrowBookRequestDto borrowBookRequestDto) throws BookException {

    Book book =
        bookRepository
            .findById(UUID.fromString(borrowBookRequestDto.bookId()))
            .orElseThrow(() -> new BookException(ExceptionMessage.EXP_MSG_BOOK_NOT_AVAILABLE));
    Borrower borrower =
        borrowerRepository
            .findById(UUID.fromString(borrowBookRequestDto.borrowerId()))
            .orElseThrow(
                () -> new BorrowerException(ExceptionMessage.EXP_MSG_BORROWER_NOT_AVAILABLE));
    ...
    boolean alreadyBorrowed =
        book.getCopies().stream()
            .anyMatch(
                copy ->
                    Boolean.TRUE.equals(copy.getIsBorrowed())
                        && copy.getBookStatus().equals(BookStatus.BORROWED)
                        && copy.getBorrowedBy() != null
                        && copy.getBorrowedBy()
                            .getId()
                            .toString()
                            .equals(borrowBookRequestDto.borrowerId()));
    if (alreadyBorrowed) {
      throw new LibraryOperationException(
          ExceptionMessage.EXP_MSG_BORROW_FAILED, HttpStatus.CONFLICT);
    }

    // Manually fetching the book copy using a native query with FOR UPDATE to lock the record until
    // the transaction completes
    BookCopy availableBookCopyToBorrow =
        bookCopyRepository
            .findFirstAvailableCopyForUpdate(book.getId())
            .orElseThrow(() -> new BookException(ExceptionMessage.EXP_MSG_NO_COPIES_AVAILABLE));
    ...
    borrowerHistoryRepository.save(
        borrowingHistoryMapper.buildBorrowingHistory(availableBookCopyToBorrow, borrower));

    return responseBuilder.success(
        libraryOperationMapper.buildBorrowResponseDto(book, borrower, availableBookCopyToBorrow),
        RPM_BOOK_BORROWED_SUCCESS);
  }

  /**
   * Allows a borrower to return a previously borrowed book.
   *
   * <p>Flow: 1. Validate that both book and borrower exist. 2. Check if borrower has any active
   * (non-returned) borrowing history. 3. Ensure the borrowed book matches the requested book to
   * return. 4. Mark the borrowing history as returned and update the book copy’s status. 5. Return
   * a success response with return details.
   *
  ...
  public ResponseEntity<ApiResponse<ReturnBookResponseDto>> returnBook(
      ReturnBookRequestDto returnBookRequestDto) throws BookException {
    Book book =
        bookRepository
            .findById(UUID.fromString(returnBookRequestDto.bookId()))
            .orElseThrow(() -> new BookException(ExceptionMessage.EXP_MSG_BOOK_NOT_AVAILABLE));
    Borrower borrower =
        borrowerRepository
            .findById(UUID.fromString(returnBookRequestDto.borrowerId()))
            .orElseThrow(
                () -> new BorrowerException(ExceptionMessage.EXP_MSG_BORROWER_NOT_AVAILABLE));

    List<BorrowingHistory> borrowingHistory =
        borrowerHistoryRepository.findBorrowingHistoryByBorrowerAndReturnedAtIsNull(borrower);
    ...
    BorrowingHistory bHistory =
        borrowingHistory.stream()
            .filter(
                bh -> bh.getBookCopy().getBook().getId().toString().equals(book.getId().toString()))
            .findFirst()
            .orElseThrow(() -> new LibraryOperationException(EXP_MSG_TRY_RETURN_DIFFERENT_BOOK));
    ...
    return responseBuilder.success(
        libraryOperationMapper.buildReturnResponseDto(
            bookCopy.getBook(), borrower, bookCopy, bHistory),
        RPM_BOOK_RETURNED_SUCCESS);
  }
}
